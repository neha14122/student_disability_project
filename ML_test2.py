#features and ROC aurracy(basically accuracy)
# small_data_pipeline.py
import pandas as pd
import numpy as np
...
if udise_col and aishe_col:
    ...
    raise ValueError("UDISE/AISHE total columns not found")
...
# -----------------------------
# 3️⃣ Handle nulls and numeric conversion
# -----------------------------
...
# -----------------------------
# 4️⃣ Create binary target for Early Intervention
# -----------------------------
...
primary["Gap_ParentalInteraction"] = primary["Gap_Risk_Index"] * (primary["Parental_Involvement"].map({"Low":1,"Medium":2,"High":3}).fillna(0))
...
# Binning numeric features (optional)
primary["Survival_Bin"] = pd.cut(primary["Survival_to_Class10"], bins=[0,50,100,150,300], labels=[0,1,2,3])
...
# -----------------------------
# 7️⃣ Define targets and features
# -----------------------------
...
# -----------------------------
# 8️⃣ Training Helper with SMOTE + CV
# -----------------------------
def train_model_small_data(X, y, task_name):
    ...
    for train_idx, test_idx in kf.split(X, y):
        ...
        X_train_bal, y_train_bal = smote.fit_resample(X_train_filled, y_train_filled)
        ...
        # Random Forest
        rf = RandomForestClassifier(n_estimators=200, max_depth=6, min_samples_leaf=5, class_weight="balanced", random_state=42)
    ...
    rf.fit(X, y)  # Fit final model on all data
...
# -----------------------------
# 10️⃣ Colorful Feature Importance
# -----------------------------
